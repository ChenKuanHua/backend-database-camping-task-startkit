...
SELECT id, (SELECT id FROM "SKILL" WHERE name = '重訓') FROM "COACH";

INSERT INTO "COACH_LINK_SKILL" (coach_id, skill_id)
VALUES ((SELECT id FROM "COACH" WHERE user_id = (SELECT id FROM "USER" WHERE name = '肌肉棒子')),
        (SELECT id FROM "SKILL" WHERE name = '瑜伽'));

INSERT INTO "COACH_LINK_SKILL" (coach_id, skill_id)
VALUES ((SELECT id FROM "COACH" WHERE user_id = (SELECT id FROM "USER" WHERE name = 'Q太郎')),
...
SELECT "USER".name AS 購買人, sum("CREDIT_PURCHASE".purchased_credits) AS 累計購買堂數
...
select "USER".name as 購買人, COUNT("COURSE_BOOKING".booking_at) as 已使用堂數
from "COURSE_BOOKING"
inner join "USER" on "COURSE_BOOKING".user_id = "USER".id
where user_id = (select id from "USER" where name = '王小明') AND status NOT LIKE '課程已取消'
group by "USER".id;
...
SELECT 
    "CREDIT_PURCHASE".user_id as 購買人,
...
INNER JOIN 
    ...
    (
        SELECT 
            user_id, 
            COUNT(booking_at) AS used_credit
        FROM "COURSE_BOOKING"
        WHERE status NOT LIKE '課程已取消'
...
SELECT "SKILL".name, count("COACH_LINK_SKILL".skill_id) AS coach_total
FROM "COACH"
INNER JOIN "COACH_LINK_SKILL" ON "COACH_LINK_SKILL".coach_id = "COACH".id 
INNER JOIN  "SKILL" ON  "SKILL".id = "COACH_LINK_SKILL".skill_id
GROUP BY "SKILL".name
...
WHERE 
    ...
    AND DATE_PART('year', "CREDIT_PURCHASE".purchase_at) = 2024
...
FROM "CREDIT_PURCHASE"
WHERE 
    DATE_PART('month', purchase_at) = 11
    AND DATE_PART('year', purchase_at) = 2024;

-- 6-5. 查詢：計算 11 月份有預約課程的會員人數（需使用 Distinct，並用 created_at 和 status 欄位統計）
-- 顯示須包含以下欄位： 預約會員人數
SELECT COUNT(DISTINCT user_id) AS 十一月預約會員人數
FROM "COURSE_BOOKING"
WHERE 
    DATE_PART('month', booking_at) = 11
    AND DATE_PART('year', booking_at) = 2024
